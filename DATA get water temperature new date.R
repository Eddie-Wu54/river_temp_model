...
wt <- brick("gfdl_rcp8p5_2006-01-07_to_2019-12-30.nc", varname = "waterTemperature")
weeklyWater <- extract(wt, locations.spatial)


# Change from Kelvin to Celsius
M.weeklyWater <- weeklyWater - 273.15 # should have 728 columns of data
...
# standard deviation, max, and min for each week of the year (2006-2019)
...
# Use two for loops to calculate the average for each week over 14 years
for (i in 1:52) { # i index which week
  
  # sub setting that week for each year
  subset <- data.frame(M.weeklyWater[,i])
  
  for (j in 1:13) { # j index which year
    subset <- cbind(subset, M.weeklyWater[,i+52*j])
  } # now "subset" is a data frame that contains the 14-year temperature data for week i
  
  # Calculate the mean
  week.avg[,i] <- rowSums(subset)/14
  # Calculate the margin of error (for confidence interval)
  for (y in 1:14) { # y index each location
    standard_error <- sd(subset[y,]) / sqrt(14)
...
write.csv(st.louis.mod, "weekly modeled water temperature_new/st_louis_model.csv")
...
write.csv(saginaw.mod, "weekly modeled water temperature_new/saginaw_model.csv")
...
write.csv(fox.mod, "weekly modeled water temperature_new/fox_model.csv")
...
write.csv(pb.mod, "weekly modeled water temperature_new/pb_model.csv")
...
write.csv(vermilion.mod, "weekly modeled water temperature_new/vermilion_model.csv")
...
write.csv(genesee.mod, "weekly modeled water temperature_new/genesee_model.csv")
...
write.csv(big.creek.mod, "weekly modeled water temperature_new/bigcreek_model.csv")
...
write.csv(big.otter.mod, "weekly modeled water temperature_new/bigotter_model.csv")
...
write.csv(still.mod, "weekly modeled water temperature_new/still_model.csv")
...
write.csv(mississagi.mod, "weekly modeled water temperature_new/mississagi_model.csv")
...
write.csv(nipigon.mod, "weekly modeled water temperature_new/nipigon_model.csv")
...
write.csv(humber.mod, "weekly modeled water temperature_new/humber_model.csv")
...
write.csv(lp.mod, "weekly modeled water temperature_new/lp_model.csv")
...
write.csv(portdover.mod, "weekly modeled water temperature_new/portdover_model.csv")
