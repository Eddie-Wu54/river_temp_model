...
lag2.plot(master.annual$air, master.annual$water, 5)
```
...
## Plot ACF
ggarrange(plot(ACF(model,resType="n"),alpha=0.05),
...
dw_test_nlme <- dwtest(resid(model.ar)~ 1)
print(dw_test_nlme)
```
...
dw_test_nlme <- dwtest(resid(residual.comp)~ 1)
print(dw_test_nlme)
```
...
current.training <- current.training %>% 
  rowwise() %>% 
  mutate(cair = mean(c_across(9:14)),
         ldair = (6/16*air+5/16*dmean_1+4/16*dmean_2+
                  3/16*dmean_3+2/16*dmean_4+1/16*dmean_5))
...
# make sure to change summer coefficient when fitting models on seasons
#coef.summer <- c(alpha=25, gamma=7,beta=10)
#coef.spring <- c(alpha=20, gamma=5,beta=9)


# model training and predicting
model <- nlme(water ~ alpha / (1 + exp(gamma * (beta - air))),
              fixed = list(alpha~1,gamma~1,beta~1),
              random = gamma ~ 1|location,
              start = c(alpha=26,gamma=7,beta=10),
              data = current.training, na.action = na.omit,
              control = list(msMaxIter = 200))

compare$preds <- as.vector(predict(model, newdata = current.testing))

# new model with new air temp
model.new <- nlme(water ~ alpha / (1 + exp(gamma * (beta - cair))),
              fixed = list(alpha~1,gamma~1,beta~1),
              random = gamma ~ 1|location,
              start = c(alpha=26,gamma=7,beta=10),
              data = current.training, na.action = na.omit,
...
compare$preds.new <- as.vector(predict(model.new, newdata = current.testing))
...
for (loc in loc_seq) {
  plot.df <- compare[compare$location == loc,]
  diff <- round(sqrt(mean((plot.df$preds-plot.df$water)^2)),2)
  diff.new <- round(sqrt(mean((plot.df$preds.new-plot.df$water)^2)),2)
  ...
  pl <- ggplot(data=plot.df, aes(x=date))+
          ...
          ggtitle(paste(
            loc, ": ", plot.df$year, " (RMSE:", diff, "&", diff.new, ")"))
  print(pl)
}
...
dw_test_nlme <- dwtest(resid(model.new)~ 1)
print(dw_test_nlme)
```
